...
global mergetempsuffix="CA_Official"
 ...
 set more off
	...
	** LLCs
	...
	infile using /projects/reap.proj/raw_data/California/CA_LPMASTER, using(/projects/reap.proj/raw_data/California/LPMASTER.TXT)
	rename id llcid
	gen ord = _n
	recast long ord 
...
****
	...
	*corp_add_recapitalizations,dta(~/final_datasets/CA.dta) merger(CA.recapitalizations.dta) matchvariable(corpnumber)
	...
	corp_add_vc2 	 CA  ,dta(~/final_datasets/CA.`dtasuffix'.dta) vc(~/final_datasets/VC.investors.dta) longstate(CALIFORNIA) skipcollapsed

	###corp_add_vc 	 CA  ,dta(~/final_datasets/CA.`dtasuffix'.dta) vc(~/final_datasets/VX.dta) longstate(CALIFORNIA) 


	corp_add_ipos	 CA  ,dta(~/final_datasets/CA.`dtasuffix'.dta) ipo(/projects/reap.proj/data/ipoallUS.dta)  longstate(CALIFORNIA) skipcollapsed
	corp_add_mergers CA  ,dta(~/final_datasets/CA.`dtasuffix'.dta) merger(/projects/reap.proj/data/mergers.dta)  longstate(CALIFORNIA) skipcollapsed
